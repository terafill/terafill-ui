...
export const CreateAccountForm = () => {
  ...
  const createAccountAction = (e) => {
    ...
    if (passwordRef.current.value != passwordRepeatRef.current.value) {
      ...
      initateSignupProcess(userData.email)
            ...
            {Object.entries(steps).map(([newStepIdx, step]) => (
              <li key={step.name} className='md:flex-1'>
                {step.status === 'completed' ? (
                  <span className='flex flex-col items-center border-l-4 border-black py-2 pl-4 md:border-l-0 md:border-t-4 md:pb-0 md:pl-0 md:pt-4'>
                    <span className='text-sm font-medium text-black'>{step.name}</span>
                  </span>
                ) : step.status === 'current' ? (
                  <span className='flex flex-col justify-center items-center border-l-4 border-black py-2 pl-4 md:border-l-0 md:border-t-4 md:pb-4 md:pl-0 md:pt-4 bg-gray-200 rounded-lg'>
                    <span className='text-sm font-medium text-black'>{step.name}</span>
                  </span>
                ) : (
                  <span className='flex flex-col items-center border-l-4 border-gray-200 py-2 pl-4 md:border-l-0 md:border-t-4 md:pb-0 md:pl-0 md:pt-4'>
