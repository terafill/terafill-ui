...
export const storeAuthData = (
  email,
  password=null,
  key_wrapping_key=null
) => {
  localStorage.clear();
  localStorage.setItem('email', email);
  if(key_wrapping_key){
    console.log("storeAuthData.key_wrapping_key", key_wrapping_key);
    const hashed_password = getHash(password, "SHA-256");
    const key_wrapping_key_decypted = forge.pki.decryptRsaPrivateKey(key_wrapping_key, hashed_password);
    const key_wrapping_key_pem = forge.pki.privateKeyToPem(key_wrapping_key_decypted);
    // const hashed_key_wrapping_key_pem = getHash(key_wrapping_key_pem, "SHA-256");
    // localStorage.setItem('key_wrapping_key', hashed_key_wrapping_key_pem);
    localStorage.setItem('key_wrapping_key', key_wrapping_key_pem);

    const key_wrapping_key_public = forge.pki.setRsaPublicKey(key_wrapping_key_decypted.n, key_wrapping_key_decypted.e);
